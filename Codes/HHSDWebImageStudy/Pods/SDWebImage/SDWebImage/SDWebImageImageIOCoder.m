...
- (UIImage *)decompressedImageWithImage:(UIImage *)image
    ...
    if (!shouldScaleDown) {
        // 解压 sd_decompressedImageWithImage:
        return [self sd_decompressedImageWithImage:image];
    } else {
        // 解压并缩放 sd_decompressedAndScaledDownImageWithImage:
...
- (nullable UIImage *)sd_decompressedImageWithImage:(nullable UIImage *)image {
    ...
    // 新建自动释放池，将bitmap context和临时变量都添加到池中 以备 末尾自动释放(防止内存警告)😎
    // autorelease the bitmap context and all vars to help system to free memory when there are memory warning.
    // on iOS7, do not forget to call [[SDImageCache sharedImageCache] clearMemory];
    
    @autoreleasepool{
        
        // 传入的 UIImage 对应的 CGImageRef（位图）
        CGImageRef imageRef = image.CGImage;
        
        // device color space 获取设备的色彩空间
        ...
        UIImage *imageWithoutAlpha = [[UIImage alloc] initWithCGImage:imageRefWithoutAlpha
                                                                scale:image.scale
