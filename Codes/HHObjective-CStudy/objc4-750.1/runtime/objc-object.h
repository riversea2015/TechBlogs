...
{
    if (!isTaggedPointer()) return ISA(); // 如果不是 taggedPointer，则执行 ISA()
...
// 判断是否是 Tagged Pointer，若最高位是1（iOS）或最低位是1（Mac OS），则就是 Tagged Pointer
inline bool 
...
{
    assert(!isTaggedPointer());  // 如果是 TaggedPointer 就会中断言
#if SUPPORT_INDEXED_ISA // Define SUPPORT_INDEXED_ISA=1 on platforms that store the class in the isa field as an index into a class table.
    if (isa.nonpointer) {
