...
// 核心方法
// *** cats 是一个指向结构体 category_list 的指针
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // 1.Discover load methods
    {
        ...
        prepare_load_methods((const headerType *)mh); // mh -> mac header
    }
...
# endif
    ...
    // Discover categories.
    // for 里边是针对一个类
    for (EACH_HEADER) { // 展开就是 for (hIndex = 0; hIndex < hCount && (hi = hList[hIndex]); hIndex++) {
        // *** 得到一个一维数组，其中每个元素是一个分类的地址，count 是分类地址的个数 [category_t *, category_t *, category_t *]
        category_t **catlist = _getObjc2CategoryList(hi, &count);
        bool hasClassProperties = hi->info()->hasCategoryClassProperties();

        // for 里边是针对一个类的一个分类
        for (i = 0; i < count; i++) {
            category_t *cat = catlist[i]; // *** cat 是一个指向结构体的指针
...
static void schedule_class_load(Class cls)
{
    ...
    // 如果已经执行过 load 方法，不再往下执行
    if (cls->data()->flags & RW_LOADED) return;

    // 递归调用，目的：先找父类的，再找子类的
    schedule_class_load(cls->superclass);

    // 将含有 load 方法的类及对应的 load 方法存储到 loadable_list 里边
...
#warning fixme build and search caches
    ...
    lookUpImpOrNil(cls, sel, nil, NO/*initialize*/, NO/*cache*/, YES/*resolver*/); // 点开
                       ...
                       bool initialize, bool cache, bool resolver) // 🍎 9.
{
    ...
    // 2.如果需要执行 +initialize，并且当前类没有执行过 +initialize，就去执行 initialize 方法
    if (initialize  &&  !cls->isInitialized()) {
        runtimeLock.unlock();
        _class_initialize (_class_getNonMetaClass(cls, inst)); // 点开
        runtimeLock.lock();
 ...
 retry:    
    ...
    // 1.查缓存 Try this class's cache.
    
    imp = cache_getImp(cls, sel);
    if (imp) goto done;

    // 2.查方法列表 Try this class's method lists.
    ...
    // 3.查父类的缓存和方法列表 Try superclass caches and method lists.
...
{
    IMP imp = lookUpImpOrForward(cls, sel, inst, initialize, cache, resolver); // 点开
