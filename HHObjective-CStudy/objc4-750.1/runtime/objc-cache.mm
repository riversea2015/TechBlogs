...
bucket_t * cache_t::find(cache_key_t k, id receiver)
{
    ...
    mask_t begin = cache_hash(k, m); // begin = key & mask
    
    mask_t i = begin;
    do {
        if (b[i].key() == 0  ||  b[i].key() == k) { // 根据计算出来的下标 i 去哈希表表里边取对应的元素，取出里边的 _key 与传件来的 key 比较，因为不同的 key 有可能算出同一个下标
            return &b[i];
        }
    } while ((i = cache_next(i, m)) != begin); // i = (i+1) & mask
